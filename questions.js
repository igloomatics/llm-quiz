...
**准备好了吗？开始挑战！**`,
    ...
    },
    {
        ...
        difficulty: '挑战',
        order: 2,
        question: '【默写1/6】TokenEmbedding - 补全函数实现',
        code: `class TokenEmbedding(nn.Module):
    def __init__(self, vocab_size, d_model):
        # 在下方补全__init__函数
        [blank]
    ...
    def __init__(self, d_model, max_len=5000):
        super().__init__()
        # 在下方补全位置编码矩阵的创建
        # 提示: pe矩阵, position, div_term, sin/cos计算
        [blank]
        
        # 在下方注册buffer
        # 提示: self.register_buffer('pe', pe)
        [blank]
    ...
    },
    {
        ...
        difficulty: '挑战',
        order: 4,
        question: '【默写3/6】ScaledDotProductAttention - 补全实现',
        code: `class ScaledDotProductAttention(nn.Module):
    def __init__(self, dropout=0.1):
        # 在下方补全__init__函数
        [blank]
    ...
    def forward(self, Q, K, V, mask=None):
        # 在下方补全forward函数：计算attention
        # 提示: QK^T / √d_k → mask → softmax → dropout → 乘V
        [blank]`,
        answers: ['init', 'forward'],
    ...
    def __init__(self, d_model, num_heads, dropout=0.1):
        # 在下方补全初始化
        # 提示: W_Q, W_K, W_V, W_O线性层, ScaledDotProductAttention
        [blank]
    
    def forward(self, Q, K, V, mask=None):
        # 在下方补全forward
        # 提示: 投影→view分头→transpose→attention→transpose→view合并→输出投影
        [blank]`,
        answers: ['init', 'forward'],
    ...
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1):
        # 在下方补全初始化
        # 提示: self_attn, ffn, norm1, norm2, dropout1, dropout2
        [blank]
    
    def forward(self, x, mask=None):
        # 在下方补全forward
        # 提示: Self-Attention → Add&Norm → FFN → Add&Norm
        [blank]`,
        answers: ['init', 'forward'],
    ...
    def __init__(self, vocab_size, d_model, num_heads, d_ff, 
                 num_layers, dropout=0.1, max_len=5000):
        # 在下方补全初始化
        # 提示: token_embedding, pos_encoding, layers(ModuleList), dropout
        [blank]
    
    def forward(self, src, src_mask=None):
        # 在下方补全forward
        # 提示: embedding → pos_encoding → dropout → 遍历layers
        [blank]`,
        answers: ['init', 'forward'],
